"""
ReelRecon - Comprehensive Logging System
...
class ReelReconLogger:
    ...
    def __init__(self, log_dir: Optional[Path] = None, max_file_size_mb: int = 10, max_files: int = 5):
        ...
        # Current log file
        self.current_log_file = self.log_dir / "reelrecon.log"
        self.error_log_file = self.log_dir / "errors.log"

        # Error registry for tracking unique errors
    ...
    def _generate_error_code(self, category: str, message: str) -> str:
        """Generate unique, trackable error code"""
    ...
    def error(self, category: str, message: str, data: Optional[Dict] = None,
        ...
        """
        Log error with auto-generated error code.
        Returns the error code for display to users.
        """
    ...
    def critical(self, category: str, message: str, data: Optional[Dict] = None,
                 exception: Optional[Exception] = None) -> str:
        """Log critical error. Returns error code."""
        error_code = self._generate_error_code(category, message)
    ...
    def get_error_details(self, error_code: str) -> Optional[Dict]:
        """Retrieve details for a specific error code"""
    ...
    def scrape_event(self, scrape_id: str, event: str, data: Optional[Dict] = None):
        """Log scrape-specific event"""
        log_data = {"scrape_id": scrape_id}
        if data:
            log_data.update(data)
        self.info("SCRAPE", event, log_data)

    def progress(self, scrape_id: str, phase: str, progress_pct: int, message: str):
        """Log progress update"""
        self.info("PROGRESS", message, {
            "scrape_id": scrape_id,
            "phase": phase,
...
_logger: Optional[ReelReconLogger] = None


def get_logger() -> ReelReconLogger:
    """Get the global logger instance"""
    global _logger
    if _logger is None:
        _logger = ReelReconLogger()
    return _logger
